...
7.	Display all information of products. Condition: Products were not sold.
...
-- 1.Display information includes: pro_id, pro_name, sup_name. Sorted in descending product_id.
select PRO_ID, PRO_NAME, SUP_NAME
from PRODUCT
inner join SUPPLIER on SUPPLIER.SUP_ID = PRODUCT.SUP_ID
...
-- 3.Display all information of products in ‘Thai Lan’ or ‘Viet Nam’
select * from PRODUCT
where COUNTRY = 'Thai Lan' or COUNTRY = 'Viet Nam';
...
-- 6.Display information includes: ORD_ID, CUS_NAME, every orders display sum of money (quantity x price)
select ORDERS.ORD_ID, CUS_NAME, sum(QUANTITY * PRICE) as Total
from ORDERS
inner join CUSTOMER on ORDERS.CUS_ID = CUSTOMER.CUS_ID
inner join DETAIL on ORDERS.ORD_ID = DETAIL.ORD_ID
inner join PRODUCT on DETAIL.PRO_ID = PRODUCT.PRO_ID
GROUP BY ORDERS.ORD_ID, CUSTOMER.CUS_NAME;

-- 7.Display all information of products. Condition: Products were not sold.
select * from PRODUCT
where PRO_ID not in (select PRO_ID from DETAIL);

-- 8.Display information of orders: ORD_ID, CUS_NAME, the total quantity. Condition: The total quantity purchased is the highest.
select ORDERS.ORD_ID, CUSTOMER.CUS_NAME, SUM(DETAIL.QUANTITY) as TotalQuantity
FROM ORDERS 
JOIN Customer ON ORDERS.CUS_ID = CUSTOMER.CUS_ID
JOIN DETAIL ON ORDERS.ORD_ID = DETAIL.ORD_ID
GROUP BY ORDERS.ORD_ID, CUSTOMER.CUS_NAME
having SUM(DETAIL.QUANTITY) = (
...
-- 9.Display information of suppliers: SUP_ID, SUP_NAME, Count of Products. Condition: Count of Products > =2
select SUPPLIER.SUP_ID, SUP_NAME, count(PRO_ID)
from SUPPLIER
inner join PRODUCT on SUPPLIER.SUP_ID = PRODUCT.SUP_ID
group by SUPPLIER.SUP_ID, SUP_NAME
